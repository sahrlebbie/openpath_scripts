...
sudo -u splunk mkdir /opt/splunkforwarder/etc/apps/$APPS_DS_APP
sudo -u splunk mkdir /opt/splunkforwarder/etc/apps/$APPS_DS_APP/local
sudo -u splunk touch /opt/splunkforwarder/etc/apps/$APPS_DS_APP/local/deploymentclient.conf
...
sudo -u splunk echo "[deployment-client]" > /opt/splunkforwarder/etc/apps/$APPS_DS_APP/local/deploymentclient.conf

sudo -u splunk echo "[target-broker:deploymentServer]" >> /opt/splunkforwarder/etc/apps/$APPS_DS_APP/local/deploymentclient.conf
sudo -u splunk echo "targetUri= $DEPLOYMENTSERVER_IP:8089" >> /opt/splunkforwarder/etc/apps/$APPS_DS_APP/local/deploymentclient.conf
